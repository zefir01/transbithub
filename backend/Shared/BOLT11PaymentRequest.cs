...
namespace Shared
{
    public class InternalBech32Encoder
    {

        private static readonly InternalBech32Encoder _Instance = new InternalBech32Encoder();
        public static InternalBech32Encoder Instance
        {
            get
            {
                return _Instance;
  ...
  626, 978, 825, 344, 605, 654, 209
        ...
        {
            var len = hrp.Length;
            var _HrpExpand = new byte[(2 * len) + 1];
            for (int i = 0; i < len; i++)
            {
        ...
        bool VerifyChecksum(byte[] data, int bechStringLen, byte[] hrpExpand, out int[] errorPosition)
        {
            errorPosition = null;
            var values = Concat(hrpExpand, data);
        ...
        internal static byte[] Concat(byte[] arr, params byte[][] arrs)
        {
        ...
        public (string HumanReadablePart, byte[] Data) DecodeData(string encoded)
        {
            ...
            if (pos < 1 || pos + 7 > encoded.Length)
            {
                throw new FormatException("bech missing separator, separator misplaced");
            }
            ...
            int[] error;
            if (!VerifyChecksum(data, encoded.Length, GetHrpExpand(hrp), out error))
            {
                if (error.Length == 0)
